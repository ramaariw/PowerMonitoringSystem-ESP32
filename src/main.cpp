...
void callback(char* topic, byte* payload, unsigned int len) {
    ...
    if (t == "esp32rm/r1/cmd") {
        ...
        scheduleR1 = (msg == "OFF") ? "" : msg;
    }
    else if (t == "esp32rm/r2/schedule") {
        scheduleR2 = (msg == "OFF") ? "" : msg;
    }
...
void setup() {
    ...
    tampilkanIntroLCD("PMS V1.3 READY");

    setup_wifi(); // Pakai WiFiManager Non-Blocking lo
...
void loop() {
    ...
    // --- 5. PEMBACAAN SENSOR & BROADCAST (Tiap 3 Detik) ---
    if (currentMillis - previousMillis >= 3000) {
        ...
        // KIRIM DATA HANYA VIA MQTT (Lokal HTTP dihapus biar kenceng)
